...
class ScenesOCRTests: XCTestCase {
    ...
    func testOCRScene22() {
        OCRHelper.execute(self, scene: .sc22, exlude: [], isLow: false) { (answer, position) in
            XCTAssertEqual(position.letter.value, answer, OCRHelper.message(position))
        }
    }

    func testOCRScene23() {
        OCRHelper.execute(self, scene: .sc23, exlude: [], isLow: false) { (answer, position) in
            XCTAssertEqual(position.letter.value, answer, OCRHelper.message(position))
        }
    }

    func testOCRScene24() {
        OCRHelper.execute(self, scene: .sc24, exlude: [], isLow: false) { (answer, position) in
            XCTAssertEqual(position.letter.value, answer, OCRHelper.message(position))
        }
    }

    func testOCRScene25() {
        OCRHelper.execute(self, scene: .sc25, exlude: [], isLow: false) { (answer, position) in
            XCTAssertEqual(position.letter.value, answer, OCRHelper.message(position))
        }
    }

    func testOCRScene26() {
        OCRHelper.execute(self, scene: .sc26, exlude: [], isLow: false) { (answer, position) in
            XCTAssertEqual(position.letter.value, answer, OCRHelper.message(position))
        }
    }
